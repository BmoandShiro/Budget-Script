...
//var SETTINGS = new Object();
  ...
  currentSetting.maxCost = getFloat("99999999");
  ...
  currentSetting.blockReEnableIfLabelContains = [
    "stopped by holiday exclusions script",
    "stopped by day schedule script"
  ];
 ...
 case "Monthly":
  ...
  if(currentSetting.scope.toLowerCase().indexOf("campaign") != -1 || currentSetting.scope.toLowerCase().indexOf("account") != -1) {
    ...
    while(iterator.hasNext()){
      var item = iterator.next();
      var name = item.getName();
      if (shouldSkipReEnableForOtherScriptLabels(item, name)) continue;
  ...
  if(currentSetting.scope.toLowerCase().indexOf("ad group") != -1) {
    ...
    while(iterator.hasNext()){
      ...
      var name = item.getName();
      var campaignName = item.getCampaign().getName();
      if (shouldSkipReEnableForOtherScriptLabels(item, "campaign: '" + campaignName + "' ad group: '" + name + "'")) continue;
    ...
    while(adIterator.hasNext()) {
      ...
      var displayUrl = ad.getDisplayUrl();
      var fullAdText = adHeadline + " " + description1 + " " + description2 + " " + displayUrl;
      if (shouldSkipReEnableForOtherScriptLabels(ad, fullAdText)) continue;
      ad.removeLabel(currentSetting.labelToAdd);
     
      Logger.log("Enabling ad: " + fullAdText);
    ...
    while(iterator.hasNext()){
      ...
      var campaignName = item.getCampaign().getName();
      var adGroupName = item.getAdGroup().getName();
      var keywordText = "campaign: '" + campaignName + "' ad group: '" + adGroupName + "' kw: '" + name + "'";
      if (shouldSkipReEnableForOtherScriptLabels(item, keywordText)) continue;
      Logger.log("Enabling " + keywordText);
      item.removeLabel(currentSetting.labelToAdd);
     
     
      item.enable();
      currentSetting.enabledCounter++;
      currentSetting.enabledList.push(keywordText);
     
    } // while iterator hasnext
  } // if scope == keyword
}

function shouldSkipReEnableForOtherScriptLabels(item, itemName) {
  if (!currentSetting.blockReEnableIfLabelContains || currentSetting.blockReEnableIfLabelContains.length === 0) {
    return false;
  }

  for (var i = 0; i < currentSetting.blockReEnableIfLabelContains.length; i++) {
    var needle = currentSetting.blockReEnableIfLabelContains[i];
    if (entityHasLabelNameContaining(item, needle)) {
      Logger.log(
        "Skipping re-enable because blocking label is still present ('" +
          needle +
          "'): " +
          itemName
      );
...
function entityHasLabelNameContaining(item, needle) {
  ...
  var n = needle.toString().toLowerCase();
  try {
    var it = item.labels().get();
    while (it.hasNext()) {
      var labelName = it.next().getName();
      if (labelName && labelName.toString().toLowerCase().indexOf(n) !== -1) return true;
    }
  } catch (e) {
    Logger.log("Could not read labels for entity while checking re-enable blockers.");
  }
  return false;
}
